...
export default function Home() {
 ...
 const settings = {
    ...
    return (
      <div >
       <CarouselHeader/>
       <div className='px-20 sticky hidden 2xl:block  bg-white py-3 carousel-icon-dad mt-3' style={{top:'88px',zIndex:'5'}}>
       <Slider {...settings}>
          <div className="carousel-icon" >
            {image1.map((item,index)=>{
              return <div key={index} className='flex flex-col items-center mr-10 cursor-pointer rounded-md hover:bg-gray-200 hoverDn'>
                <img src={item.url} alt="" className='h-7 w-7 '/>
                <span>{item.name}</span>
              </div>
...
<div className='container-dn pb-10'>
          <HomePosition/>
